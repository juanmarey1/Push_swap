/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   check_args.c                                       :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: jrey-roj <marvin@42.fr>                    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/08/26 15:21:52 by jrey-roj          #+#    #+#             */
/*   Updated: 2024/08/26 15:21:54 by jrey-roj         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "inc/push_swap.h"

void	check_args_argc_is_2(char **argv, t_stack *stack)
{
	int	i;
	int	j;

	i = -1;
	stack->args = ft_split(argv[1], ' ');
	while (stack->args[++i])
	{
		j = 0;
		if (((stack->args[i][j] == '-' || stack->args[i][j] == '+')
			&& (stack->args[i][j + 1] >= '0' && stack->args[i][j + 1] <= '9'))
			|| (stack->args[i][j] >= '0' && stack->args[i][j] <= '9'))
			j++;
		else
			ft_error(stack);
		while (stack->args[i][j])
		{
			if (!(stack->args[i][j] >= '0' && stack->args[i][j] <= '9'))
				ft_error(stack);
			j++;
		}
	}
}

char	**ft_doublestrcpy(int argc, char **argv, t_stack *stack)
{
	int		i;
	char	**double_str;

	double_str = (char **)malloc(sizeof(char *) * argc);
	if (!double_str)
		ft_error(stack);
	i = 1;
	while (argv[i])
	{
		double_str[i - 1] = ft_strdup(argv[i]);
		if (!double_str[i - 1])
		{
			ft_double_free(double_str);
			ft_error(stack);
		}
		i++;
	}
	double_str[i - 1] = NULL;
	return (double_str);
}

void	check_args_argc_not_2(int argc, char **argv, t_stack *stack)
{
	int	i;
	int	j;

	i = 0;
	while (argv[++i])
	{
		j = 0;
		if (((argv[i][j] == '-' || argv[i][j] == '+')
			&& (argv[i][j + 1] >= '0' && argv[i][j + 1] <= '9'))
			|| (argv[i][j] >= '0' && argv[i][j] <= '9'))
			j++;
		else
			ft_error(stack);
		while (argv[i][j])
		{
			if (!(argv[i][j] >= '0' && argv[i][j] <= '9'))
				ft_error(stack);
			j++;
		}
	}
	stack->args = ft_doublestrcpy(argc, argv, stack);
}

void	check_no_rep_num(char **double_str, t_stack *stack)
{
	int	len;
	int	j;
	int	stay;

	len = 0;
	while (double_str[len])
		len++;
	j = 1;
	while (--len > 0)
	{
		stay = j - 1;
		while (double_str[j])
		{
			if (ft_atoi(double_str[stay]) == ft_atoi(double_str[j]))
				ft_error(stack);
			else
				j++;
		}
		j = stay + 2;
	}
}

void	check_args_max_int(t_stack *stack)
{
	int	i;

	i = 0;
	while (stack->args[i])
	{
		if (ft_atoi_long(stack->args[i], stack) == 0 && stack->atoi_error == 1)
			ft_error(stack);
		else
			i++;
	}
}
